...
import AccountOnboardingDialog from '../../Components/MasterData/AccountOnboardingDialog.vue'
...
const initialOnboardRole = computed(() => ({ students: 'student', teachers: 'teacher', parents: 'parent' })[kind.value] || '')
const canOnboardHere = computed(() => props.master.can.onboard && (kind.value === 'users' || props.master.onboarding_roles?.some(role => role.value === initialOnboardRole.value)))
const openCreate = () => {
  if (canOnboardHere.value) { returnFocus = document.activeElement; onboardOpen.value = true }
  else openEditor()
}
const closeOnboard = () => { onboardOpen.value = false; nextTick(() => returnFocus?.focus?.()) }
    ...
    <div class="md-page">
      ...
      <section class="md-panel" aria-label="Daftar master data">
        <header class="md-panel-head">
          ...
          <div class="md-create-actions"><button v-if="master.can.create || canOnboardHere" type="button" class="button button-primary" @click="openCreate"><Plus :size="17" aria-hidden="true" /> {{ canOnboardHere ? 'Tambah akun & profil' : `Tambah ${selectedTab?.label}` }}</button><button v-if="canOnboardHere && master.can.create && ['students', 'teachers', 'parents'].includes(kind)" type="button" class="button button-secondary" @click="openEditor()">Lengkapi akun lama</button></div>
        </header>
        ...
        <div v-else class="md-table-wrap"><table class="md-table"><thead><tr><th scope="col">Identitas</th><th scope="col">Status</th><th v-if="master.records.data.some(visibleActions)" scope="col">Tindakan</th></tr></thead>
          <tbody><tr v-for="row in master.records.data" :key="row.ref">
            <td><strong>{{ row.title }}</strong><small>{{ row.subtitle }}</small><small v-if="kind === 'users' && row.role_label">Role: {{ row.role_label }}</small></td>
      ...
      <AccountOnboardingDialog :open="onboardOpen" :initial-role="initialOnboardRole" :roles="master.onboarding_roles || []" :options="master.options || {}" @close="closeOnboard" />
        ...
        <section ref="editorDialog" class="md-dialog" role="dialog" aria-modal="true" aria-labelledby="md-editor-title" @keydown.esc="closeEditor" @keydown="trapFocus($event, editorDialog)">
          ...
          <form class="md-dialog-content" @submit.prevent="save" novalidate>
            ...
            <div class="md-fields"><div v-for="field in fields" :key="field.key" class="md-field" :class="{'md-field-wide': field.type === 'textarea' || field.type === 'roles'}">
              ...
              <template v-if="field.type === 'roles'"><div :id="`md-${field.key}`" class="md-roles" role="radiogroup" aria-label="Role pengguna"><label v-for="option in field.options" :key="option.value"><input type="radio" name="md-user-role" :checked="form.roles?.[0] === option.value" :value="option.value" @change="form.roles = [option.value]" />{{ option.label }}</label></div></template>
            ...
            <div class="md-dialog-actions"><button type="button" class="button button-secondary" :disabled="form.processing" @click="closeEditor">Batal</button><button type="submit" class="button button-primary" :disabled="form.processing"><LoaderCircle v-if="form.processing" :size="17" class="md-spin" />{{ form.processing ? 'Menyimpan...' : editMode === 'create' ? 'Buat data' : 'Simpan perubahan' }}</button></div>
          </form>
        </section>
      </div>
